...
class InterventionScreen extends StatelessWidget {
  ...
  Widget _buildEmptyState(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(LucideIcons.shieldCheck, size: 80, color: AppColors.primary.withOpacity(0.1)),
          const SizedBox(height: 24),
          Text("No Active Interventions", style: Theme.of(context).textTheme.headlineMedium),
          Text("Your current risk is within normal bounds.", style: TextStyle(color: AppColors.textSecondary)),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(intervention.title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                const SizedBox(height: 4),
                Row(
                  children: [
                    Text(intervention.type.toUpperCase(), style: const TextStyle(fontSize: 10, color: AppColors.primary, fontWeight: FontWeight.bold)),
                    ...
                    Container(
                      ...
                      child: const Text("URGENT", style: TextStyle(color: AppColors.danger, fontSize: 8, fontWeight: FontWeight.bold)),
                    ),
                  ],
                ),
              ],
  ...
  Widget _buildWhyBox(Intervention intervention) {
    return GlassCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("Why this action?", style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
          const SizedBox(height: 8),
          Text(intervention.reason, style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
          const SizedBox(height: 12),
          Text("Linked Risk Score: 68", style: TextStyle(fontSize: 11, color: AppColors.primary, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
  ...
  Widget _buildStepsBox(Intervention intervention) {
    return GlassCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("Suggested Action Steps", style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
          const SizedBox(height: 16),
          ...intervention.steps.map((step) => Padding(
  ...
  Widget _buildImpactBox(Intervention intervention) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(LucideIcons.trendingDown, size: 20, color: AppColors.electricBlue),
          const SizedBox(width: 12),
          Expanded(
            child: Text(
              "Expected Impact: ${intervention.impact}",
              style: const TextStyle(color: AppColors.electricBlue, fontSize: 13, fontWeight: FontWeight.w500),
            ),
          ),
        ],
      ),
    ...
    return Column(
      children: [
        _buildMainButton("MARK AS DONE", AppColors.primaryGradient, () {
          state.updateInterventionStatus(intervention.id, InterventionStatus.done);
        }),
        const SizedBox(height: 12),
        Row(
          children: [
            Expanded(child: _buildSecondaryButton("SNOOZE", LucideIcons.clock, () {})),
            const SizedBox(width: 12),
            Expanded(child: _buildSecondaryButton("DISMISS", LucideIcons.x, () {
  ...
  Widget _buildTraceabilityBox(Intervention intervention) {
    return Center(
      child: Column(
        children: [
          Text("ID: ${intervention.id}", style: const TextStyle(fontSize: 10, color: AppColors.textMuted)),
          Text("Source: Backend Planner v1.2", style: const TextStyle(fontSize: 10, color: AppColors.textMuted)),
        ],
      ),
    );
  }
