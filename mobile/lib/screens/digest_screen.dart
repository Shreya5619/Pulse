  ...
  Widget build(BuildContext context) {
    return Consumer<AppState>(
      builder: (context, state, _) {
        ...
        final List<String> urgent = List<String>.from(digest?['urgent'] as List? ?? []);
        final List<String> important = List<String>.from(digest?['important'] as List? ?? []);
        final int noiseCount = (digest?['noiseCount'] as num?)?.toInt() ?? 0;
        final String highlight = digest?['highlight'] as String? ?? '0 notifications';

        // Determine if we have live data or are showing the placeholder
        final bool hasLiveData = digest != null &&
                      ...
                      Container(
                        ...
                        child: Text(
                          ...
                          style: GoogleFonts.outfit(
                            ...
                            color:
                                hasLiveData ? Colors.green : Colors.white30,
                            letterSpacing: 1,
                          ),
                        ),
                      ),
